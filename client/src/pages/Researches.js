  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto' }}>
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        ...
        <h1 style={{ fontSize: 'clamp(22px, 4vw, 26px)', fontWeight: 800, color: 'var(--text-primary)', margin: '0 0 8px' }}>Research Hub</h1>
        <p style={{ color: 'var(--text-secondary)', fontSize: 13 }}>Discover the latest environmental research and scientific insights</p>
      </div>
      ...
      <div className="dark-card" style={{ padding: 16, marginBottom: 20 }}>
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select
            ...
            className="dark-input"
            style={{ width: '100%', maxWidth: '100%', sm: { maxWidth: 200 } }}
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
        {[
          { icon: <BookOpen style={{ width: 18, height: 18 }} />, value: researchArticles.length, label: 'Research Articles', color: '#38bdf8' },
          { icon: <Users style={{ width: 18, height: 18 }} />, value: '25+', label: 'Contributing Researchers', color: '#34d399' },
          { icon: <TrendingUp style={{ width: 18, height: 18 }} />, value: '1.2K', label: 'Community Reads', color: '#f59e0b' },
        ].map((s) => (
          <div key={s.label} className="dark-stat-card" style={{ padding: 16 }}>
            <div className="dark-stat-icon" style={{ background: `${s.color}18`, color: s.color, width: 40, height: 40 }}>
              {s.icon}
            </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {filteredArticles.length > 0 ? (
          filteredArticles.map(article => {
            ...
            return (
              <div key={article.id} className="dark-card" style={{ padding: '18px 20px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-3 border-t border-[var(--border)]">
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--text-secondary)', flexWrap: 'wrap' }}>
                      ...
                      <Calendar style={{ width: 14, height: 14, marginLeft: 6 }} />
                      <span>{new Date(article.date).toLocaleDateString()}</span>
                    </div>
                    <button
                      onClick={() => toggleArticle(article.id)}
                      style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--accent)', fontWeight: 600, fontSize: 13, padding: 0 }}
                    >
